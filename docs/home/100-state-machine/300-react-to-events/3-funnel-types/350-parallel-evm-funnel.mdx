...
1. Use `eth_getLogs` to get the [primitives](../10-primitive-catalogue/1-introduction.md) configured to this funnel in the block range.
1. Merge the primitives fetched from the parallel funnel with the block from the underlying funnel.
...
stateDiagram-v2
    ...
    SyncMain --> ParallelEVM
    
    ParallelEVM: Parallel EVM Funnel
    state ParallelEVM {
